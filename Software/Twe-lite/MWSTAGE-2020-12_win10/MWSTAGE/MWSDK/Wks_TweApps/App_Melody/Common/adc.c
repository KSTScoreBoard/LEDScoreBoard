		...
		case E_ORDER_KICK:
			...
			if (   pObj->u8IdxMeasuruing == TEH_ADC_IDX_TEMP
				...
				if (pObj->u8InputRangeMask & u8Src) {
					// 0-1200mV
					pObj->ai16Result[pObj->u8IdxMeasuruing] = pObj->ai16Result[pObj->u8IdxMeasuruing] * 1235 / 4096;
				} else {
					// 0-2400mV
					pObj->ai16Result[pObj->u8IdxMeasuruing] = pObj->ai16Result[pObj->u8IdxMeasuruing] * 2470 / 4096;
				}
			}
